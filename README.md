...
> **v0.9.5 — security-hardening + onboarding release on the v0.9.4 pre-audit-hardening + dual-use-migration line.** 1363 tests collected (1353 green) on Ubuntu + Windows + macOS across Python 3.10 – 3.13, plus live cross-host validation on the new wire surfaces.
...
- **End-to-end source authenticity over multi-hop.** Inner Ed25519 source signatures survive per-hop re-encryption and are **verified at the destination** (v0.9.5+), binding source/destination/msg_id/payload — so a relay cannot forge, redirect, replay-relabel, or re-attribute a message; relayed frames lacking a verifiable source signature are dropped. An additional NaCl `SealedBox` end-to-end layer (X25519 keys derived from each node's identity) wraps the payload for the destination. Note: per-hop SecretBox protects each link from off-path eavesdroppers, but the plaintext body currently also rides in the per-hop layer, so a **forwarding relay can read message bodies** — making the body opaque to relays is a tracked follow-up. Route through relays you trust with contents. See [SECURITY.md](SECURITY.md).
