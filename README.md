...
- **Key encryption at rest.** Identity keys generated by `ironmesh setup` or `ironmesh keys generate` are encrypted with Argon2id. Keys auto-generated by a bare `ironmesh run` without `--keys-passphrase` are stored unencrypted and log an `INSECURE` warning — run the wizard or `keys generate` first for encrypted keys.
...
- **Key storage:** Argon2id passphrase encryption for keys created via `ironmesh setup` / `ironmesh keys generate`. A bare `ironmesh run` without `--keys-passphrase` auto-generates an unencrypted key file and logs an `INSECURE` warning.
