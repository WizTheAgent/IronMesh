...
> **v0.9.5 — security-hardening + onboarding release on the v0.9.4 pre-audit-hardening + dual-use-migration line.** 1373 tests collected (1363 green) on Ubuntu + Windows + macOS across Python 3.10 – 3.13, plus live cross-host validation on the new wire surfaces.
...
- **End-to-end encryption over multi-hop.** When the destination's identity key is known, the message body is sealed to the destination with a NaCl `SealedBox` (X25519 keys derived from each node's identity), so **only the destination can decrypt it** — a relay never holds the key. Inner Ed25519 source signatures bind source/destination/msg_id/payload and are **verified at the destination** (v0.9.5+), so a relay cannot forge, redirect, replay-relabel, or re-attribute a message. By default the sealed copy rides *alongside* the per-hop layer so the frame stays wire-compatible with every node version — meaning a relay that decrypts its per-hop layer to route can still read the plaintext. To also hide the body from relays, run with `--e2e-strict-confidentiality`, which strips the plaintext so it travels **only** in the sealed field (**a forwarding relay then cannot read message bodies**); enable it only on a mesh where every node runs v0.9.5+. If the destination's key isn't yet known, IronMesh falls back to per-hop encryption only and logs a warning. See [SECURITY.md](SECURITY.md).
