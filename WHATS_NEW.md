...
- **Pre-launch QA hardening:** a multi-round adversarial pass fixed several Reticulum/LoRa-transport issues before release — e2e messages above the LoRa cap were being silently dropped (compression collided with post-unseal source-signature verification; e2e frames are no longer compressed), the session-rekey grace window is now symmetric on both peers (closing a message-loss window during rekey on RNS's unordered large-frame path) with deterministic collision resolution, inbound LoRa decompression is bomb-capped, and `--e2e-strict-confidentiality` raises the protocol floor to `ironmesh/0.9`. See the CHANGELOG and release notes.
