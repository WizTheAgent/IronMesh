...
- **End-to-end sealing, with opt-in confidentiality from forwarding relays.**
  ...
  *alongside* the per-hop payload so the frame stays wire-compatible with every
  node version; in that default mode a relay that decrypts its per-hop layer to
  route can still read the plaintext. A new **`--e2e-strict-confidentiality`**
  flag makes the send path strip the plaintext so the body travels **solely**
  in `e2e_payload`; a relay then finds no readable body — just the opaque
  sealed field — and **cannot read message contents**. Because a relay cannot
  read a stripped body it cannot verify the inner source signature over it, so
  that verification runs at the destination after unseal (the same fail-closed
  policy: a tampered/forged/redirected sealed frame is dropped). Stripping is a
  ...
  back to per-hop-only encryption (a relay can read the body) and logs a
  warning.
...
### Fixed (pre-launch QA hardening pass)
- **`--e2e-strict-confidentiality` now enforces its own precondition.** Enabling it raises the protocol floor to `ironmesh/0.9`, so no pre-0.9 node can complete a handshake and be handed a stripped e2e frame it cannot authenticate (a pre-0.9 destination never verifies the inner source signature and would deliver the body unauthenticated). A per-peer check cannot cover multi-hop relay destinations, so the floor is the only sound enforcement; the change is logged.
- **Concurrent rekey no longer desyncs a session.** When both peers initiate a rekey at once — e.g. the periodic loop on one side and a dashboard-triggered `rotate_session` on the other, which bypasses the loop's smaller-node-id tie-breaker — the collision is now resolved deterministically in `_handle_rekey_request` (smaller node id proceeds, larger defers to it), so both converge on one key instead of black-holing the session.
- **Backup / restore write files owner-only and atomically** (temp → restrict → `os.replace`), matching `save_keys`, closing the window where a restored key/trust file briefly existed with inherited (broader) permissions or half-written after a crash.
- **RNS/LoRa: e2e messages larger than the LoRa cap are no longer silently dropped.** An end-to-end sealed frame was also LoRa-compressed, but the inner source signature is verified by the destination *after* it unseals — over the uncompressed plaintext — so the signature (computed over the compressed on-wire bytes) never matched and every such frame was dropped in the default configuration. E2E-sealed frames are no longer compressed (the uncompressed sealed copy is authoritative, so compressing the redundant per-hop copy saved nothing), keeping the sign and verify domains identical.
- **Multi-hop compressed frames are no longer dropped at the second hop.** A relay decompressed `frame.payload` in place before forwarding, invalidating the origin's signature for the next hop. LoRa payloads are now decompressed only on terminal delivery; relays forward the compressed frame untouched.
- **Decompression-bomb hardening.** Inbound LoRa-compressed payloads are inflated with a hard output ceiling (`MAX_FRAME_BYTES`); an authenticated peer can no longer expand a ~1 MiB frame into ~1 GiB to OOM a constrained node. An undecompressable or oversized frame now fails closed (drop).
- **Session rekey no longer loses in-flight messages on RNS.** The dual-key grace window was applied only on the responder; the initiator switched keys with no fallback, so a frame the peer sent under the old key that arrived after `REKEY_RESPONSE` — routine on RNS, which carries large frames on an independent, unordered Resource stream — was dropped. Both ends now open the window symmetrically, it is retired by a monotonic (NTP-step-safe) deadline rather than by the first new-key frame, and a rapid double-rekey retires the prior epoch's replay snapshot instead of orphaning it.
- **Published `docs/SECURITY.md` corrected.** The mkdocs-published Security overview was a stale copy that still claimed relays cannot read message bodies by default; it now matches the code and root `SECURITY.md` (relay-confidentiality is opt-in via `--e2e-strict-confidentiality`; default is relay-readable). A doc-sync denylist entry blocks the false phrasing from returning.
- **`SealedBox` "forward secrecy" claim corrected** in `mesh_crypto.py`, `docs/MESH.md`, `docs/SECURITY.md`, and `docs/PROTOCOL_SPEC.md`: the per-message ephemeral is the *sender's* key (sender anonymity), and the destination decrypts with its long-term X25519 secret — so it is **not** forward-secret against destination-key compromise.
- **Owner-only key-file ACL failures are now surfaced.** On Windows, a non-zero `icacls` exit (FAT/exFAT volume, unresolvable account, `icacls` off PATH, timeout) is logged at WARNING instead of being silently swallowed, so an operator — especially with `--plaintext-keys` — is not misled into thinking a key is locked down when it is not.
- **Shutdown is idempotent.** A second concurrent `shutdown()` (SIGINT then SIGTERM, or a double SIGTERM) now returns immediately, closing a narrow race where the second call's `loop.stop()` could strand the first before the message store closed, leaving the non-daemon aiosqlite worker thread un-joined and the process hanging on exit.
- **Metrics endpoint releases its socket on shutdown.** The metrics server task now holds the `asyncio` Server for its lifetime, so cancelling it on shutdown closes the listening socket (port + 1) instead of leaking it until loop close (which caused `EADDRINUSE` on an in-process restart).
- **Legacy JSON receive path is depth-guarded.** It now uses the bounded JSON parser and rejects non-object top-level values before dereferencing, so a malicious peer cannot trigger a `RecursionError`/`AttributeError` traceback per frame (log-spam), matching the binary path. Still fail-closed.
- **`stdin_is_interactive()` hardened** with explicit `ctypes` arg/return types and a broadened guard around the Windows `GetConsoleMode` probe, and bare-filename key/backup paths no longer raise `FileNotFoundError` from an unguarded `os.makedirs`.
- **Mixed-version mesh: relayed frames from upgraded nodes are no longer dropped by a legacy hop.** A pre-0.9 relay re-serializes frames through a model without the `source_sig_scheme` field, stripping the tag while preserving the bound v2 inner signature. The receiver treated a tag-absent frame as legacy v1, so the v2 signature failed to verify and the frame was dropped — silently black-holing upgraded-origin relayed traffic in a partially-upgraded mesh. The verifier now treats the scheme tag as a hint only: with the tag absent it tries the bound scheme first, then the legacy scheme when the protocol floor permits. Still fail-closed (forgery, redirect, and re-attribution remain rejected); correctness no longer depends on a relay preserving the tag.
- **`ironmesh --version`** now prints the version (was an unrecognized argument); documentation that used it as an install-verification step is correct again.
- **`ironmesh audit verify` / `audit export` resolve the identity-key passphrase from `IRONMESH_KEYS_PASSPHRASE`** (in addition to `IRONMESH_PASSPHRASE`), matching `ironmesh doctor`; a missing or corrupt key file now reports itself instead of a misleading "needs passphrase (headless)" message, and the actionable error no longer lists a `--keys-passphrase-file` flag that subcommand does not accept. A valid-JSON-but-structurally-malformed key file (empty `{}`, truncated envelope, a JSON list) now surfaces as a clean "malformed key file" error rather than an uncaught `KeyError`/`AttributeError` traceback.
- **Inner source-signature verification is fail-closed against malformed signatures.** A wrong-length signature (which PyNaCl raises as `ValueError`, not `BadSignatureError`) now returns a clean drop in-band instead of propagating out of the receive path.
- Docker image tags in `docker-compose.yml` and the `Dockerfile` build comment corrected from stale `0.8.5.8`/`0.9.2` to `0.9.5`.
- **`ironmesh backup` / `restore` no longer hang a headless run** on a hidden passphrase prompt. Both resolve the passphrase non-interactively from `IRONMESH_BACKUP_PASSPHRASE_FILE` (preferred) or `IRONMESH_BACKUP_PASSPHRASE`, falling back to an interactive prompt only on a real console and erroring out actionably otherwise.
...
- `ironmesh demo` now tears down cleanly and prints an unmistakable final `[ok]   Demo complete -- mesh handshake + encrypted ping verified.` line. Previously the mDNS close could deadlock the daemon's event loop during shutdown (hanging the process), and pending background tasks were destroyed noisily at interpreter exit; `Agent.stop()` now cancels remaining tasks and closes its event loop, and daemon shutdown bounds its server/mDNS close waits and always releases the message store. The mDNS handle no longer trails a benign `coroutine 'async_unregister_service' was never awaited` RuntimeWarning after the success line — `Zeroconf.close()` already unregisters every service, so the redundant explicit `unregister_service()` (which leaked the coroutine during the racy teardown) was removed.
