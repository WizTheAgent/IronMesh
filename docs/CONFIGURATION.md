...
| `--e2e-strict-confidentiality` | flag | off | Strip the plaintext body from end-to-end sealed frames so a relay carries only the SealedBox (relay cannot read the body). **Wire-behavior change** — enable only on a mesh where every node runs v0.9.5+; an older node that verifies the inner source but lacks the post-unseal exemption would drop a stripped frame. Off = sealed copy still attached but plaintext also rides the per-hop layer (relay-readable), fully wire-compatible. See `SECURITY.md`, "Confidentiality from forwarding relays". |
